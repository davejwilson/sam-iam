...
def _process_credentials_file(role, profile_name, aws_creds):
    with AWSTomlConfig(profile_name) as atc:
        ...
        atc.set(profile_name, "region", "af-south-1")
...
def _process_roles(roles, roles_acct_dict, saml_signoff, debug, echo_env, duration_seconds):
    ...
    return role, get_creds_via_saml_request(role, saml_signoff, debug, echo_env, duration_seconds)


def _process_roles_no_acct_info(roles, saml_signoff, debug, echo_env, duration_seconds):
    if len(roles) > 0:
        click.secho('PICK NUMBER FOR ROLES: ', fg='green')
        for idx, role in enumerate(roles):
